...
// programacion de reproductor de video (INICIO)
document.addEventListener("DOMContentLoaded", () => {
  ...
  const setPlaying = (isPlaying) => {
    ...
    // cuando está reproduciendo, el poster/botón no deben bloquear el toque
    poster.style.pointerEvents = isPlaying ? "none" : "auto";
    playBtn.style.pointerEvents = isPlaying ? "none" : "auto";
  };

  const forceStop = () => {
    video.pause();
    video.muted = true;          // corta audio sí o sí
    try { video.currentTime = 0; } catch (e) {}
    // Si algún celular sigue fallando, descomenta:
    // video.load();
    setPlaying(false);
  };

  const playVideo = () => {
    video.muted = false; // vuelve a habilitar audio al reproducir
    const p = video.play();
  ...
  // tap en el video = detener fuerte (evita “se quedó sonando”)
  video.addEventListener("click", () => {
    if (!video.paused) forceStop();
  });
  ...
  video.addEventListener("ended", forceStop);

  // si minimizan/cambian pestaña, corta el video
...
//videos de galeria 
//videos de galeria (lazy load con data-src)
document.addEventListener("DOMContentLoaded", () => {
  ...
  wrappers.forEach(wrapper => {
    ...
    let isActive = false;   // activado por click
    let isPaused = false;   // pausa manual
    let savedTime = 0;

    const loadVideo = () => {
      // Carga el mp4 SOLO la primera vez
      if (video.src) return;
    ...
    const showPoster = () => wrapper.classList.remove("is-playing");
    const hidePoster = () => wrapper.classList.add("is-playing");

    /* =====================
       🖱️ HOVER (solo escritorio)
    ===================== */
    ...
    /* =====================
       🖱️ CLICK (móvil y escritorio)
    ===================== */
    wrapper.addEventListener("click", () => {
      loadVideo();

      // PRIMER CLICK → reproduce desde 0 con audio
      if (!isActive) {
        isActive = true;
        isPaused = false;
        savedTime = 0;
...
const MOBILE_DELAY = 800; // ⏱️ ajusta: 300–800 ms
...
const io = new IntersectionObserver((entries) => {
  entries.forEach(({ isIntersecting, target }) => {
    ...
    if (isIntersecting) {
      ...
      // ⏳ delay antes de reproducir
      const t = setTimeout(() => {
        video.play().catch(() => {});
      }, MOBILE_DELAY);

      playTimers.set(video, t);
    } else {
      resetToPoster(video);
    }
  });
